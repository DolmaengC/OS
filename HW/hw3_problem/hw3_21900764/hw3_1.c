...
//		otherwise, return FALSE
{
	...
	for (int i = 0; i < strlen(file_path); i++) {
		if(file_path[i] == '/') {
			if(file_path[i+1] == '\0' || file_path[i+1] == '/') {
				printf("ERROR! incorrect directory path\n");
				return FALSE;
			}
			path_end = i - 1; 
			if(subdir_len != 0) {
			...
			new_dir_len = path_end - path_front + 1;
			subdir_len += new_dir_len;
			strncat(subdir, &file_path[path_front], new_dir_len);
			...
			if(access(subdir, access_mode) == 0) {
				...
				if(mkdir(subdir,  mkdir_mode) != 0) {
					printf("ERROR! in mkdir, subdir : %s\n", subdir);
					return FALSE;
				}
	...
	*/
	// updated code
	int access_mode = W_OK;
	mode_t mkdir_mode = 0766;
	for(int i = 0; i < strlen(file_path); i++) {
		if(file_path[i] == '/') {
			subdir[0] = '\0';
...
//		assume the destination directory exists
{
	...
	while(1) {
		...
		if(read_size <= 0)
			break;
		written_size = write(dest, buffer, strlen(buffer));
		if(written_size == -1) {
	...
	*/
	...
	while(TRUE) {
		...
		if(read_size <= 0)
			break;
		written_size = write(dest, buffer, strlen(buffer));
		printf("read_size = %zd, written_size = %zd\n", read_size, written_size);
		if(written_size == -1) {
